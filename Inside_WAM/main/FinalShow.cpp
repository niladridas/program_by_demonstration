...
//	disconnect(jpLogger.input);
	...
	string data("/home/robot/sam/myLog.csv");
	std::ifstream in(data.c_str());
	    if (!in.is_open()) return 1;

	    typedef tokenizer< escaped_list_separator<char> > Tokenizer;
	    string line;
	     //   Eigen::MatrixXd MatrixVal(1,7);
	        math::Matrix<1,DOF> MatrixVal ;

	int	j = 0;
	while (getline(in,line))
    {
	    Tokenizer tk(line, escaped_list_separator<char>('\\', ',', '\"'));
		Tokenizer::iterator i(tk.begin());

	//		double timeStamp = convertToDouble(*i) ;
		double timeStamp = atof ((*i).c_str()) ;
		//std::cout << timeStamp << std::endl;
		j = 0;
		for (++i;i!=tk.end();++i)
			{
			//double doubleValue
	   		MatrixVal(0,j) = atof ((*i).c_str()) ;
	   			j++;
			}
		jp_sample_type dataTuple = boost::make_tuple(timeStamp,MatrixVal);
		vec.push_back(dataTuple);
		//vec.push_back(boost::make_tuple(timeStamp,MatrixVal));
		//std::cout << boost::get<0>(dataTuple) << "" << boost::get<1>(dataTuple) << std::endl ;
		//std::cout << timeStamp << " " << MatrixVal << std::endl;
		//std::cout << MatrixVal(0,0) << std::endl ;
